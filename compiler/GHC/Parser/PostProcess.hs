...
getMonoBind (L loc1 (FunBind { fun_id = fun_id1@(L _ f1)
  ...
  where
    ...
    go mtchs loc binds doc_decls
        = ( L loc (makeFunBind fun_id1 tvbinds (mkLocatedList $ reverse mtchs))
...
isFunLhs e = go e [] [] []
 where
   go (L _ (PatBuilderVar (L loc f))) tyvars es ann
       | not (isRdrDataCon f)        = return (Just (L loc f, Prefix, tyvars, es, ann))
   go (L _ (PatBuilderApp f e)) tyvars es       ann = go f tyvars (e:es) ann
   go (L l (PatBuilderPar e _an)) tyvars es@(_:_) ann
                                      = go e tyvars es (ann ++ mkParensEpAnn (locA l))
   go (L loc (PatBuilderOpApp l (L loc' op) r (EpAnn loca anns cs))) tyvars es ann
        | not (isRdrDataCon op)         -- We have found the function!
        = return (Just (L loc' op, Infix, tyvars, (l:r:es), (anns ++ ann)))
        | otherwise                     -- Infix data con; keep going
        = do { mb_l <- go l tyvars es ann
             ; case mb_l of
                 Just (op', Infix, tyvars, j : k : es', ann')
                   -> return (Just (op', Infix, tyvars, j : op_app : es', ann'))
                   where
                     op_app = L loc (PatBuilderOpApp k
                               (L loc' op) r (EpAnn loca anns cs))
                 _ -> return Nothing }
   go (L _ (PatBuilderAppType e _ (HsPS {hsps_body = L l (HsTyVar _ _ c)}))) tyvars es ann = do
     mb_e <- go e tyvars es ann
     case mb_e of
       Just (e', Prefix, tyvars', es', ann') -> return $ Just (e', Prefix, tyvar : tyvars', es', ann')
       _ -> return Nothing
     where tyvar = L l $ UserTyVar noAnn SpecifiedSpec c
