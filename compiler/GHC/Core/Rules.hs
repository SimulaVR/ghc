...
matching.  So when matching
   Template {x}  f (g x)
   Target        f (g x |> co)
we want to drop the coercion.
-}
...
substitute [a/b] in the template, and then erroneously succeed in
...
In the template the cast is always a /variable/.  Matching should bind that
...
You might wonder waht we have an eta-expanded target, but regardless of
